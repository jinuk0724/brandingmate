...
function getBrandInput() {
  ...
  const type = fields.businessType.value.trim() || "로컬 사업장";
...
function generatePromoIdeas(input) {
  const imageNote = uploadedImages.length
    ? `참고 이미지 ${uploadedImages.length}장을 프롬프트의 공간, 상품, 조명, 질감 기준으로 반영`
    : "참고 이미지를 추가하면 공간, 상품, 조명, 질감 조건을 더 구체적으로 반영할 수 있음";

  const baseIdeas = [
    `리뷰 요청 카드: "${input.name}에서의 순간이 좋았다면, 짧은 리뷰로 남겨주세요."`,
    `안내문: 딱딱한 공지 대신 ${input.mood}에 맞춘 짧은 문장형 안내`,
    `SNS 포스트: 방문 전 기대감을 만드는 이미지 3장 구성`,
    `볼펜/텀블러/수건: 고객 접점에서 반복 노출되는 실사용 굿즈`,
...
function generateImagePrompt(input) {
  ...
  const corePrompt = `${input.name}의 ${goals}를 제작한다. 사업장 종류는 ${input.type}, 분위기는 ${input.mood}, 주요 고객은 ${input.audience}. ${referenceContext} 핵심 강점은 "${input.strengths}"이다. 결과물은 실제 브랜드 캠페인에 사용할 수 있는 완성형 비주얼이어야 하며, 저가 템플릿이나 단순 목업처럼 보이면 안 된다.`;
  ...
  return `
    ...
    <div class="prompt-block">
      ...
      ${renderList([
        ...
        "브랜드명 또는 대표 문구는 상단, 하단, 제품 표면 중 한 곳에만 절제해서 사용한다.",
      ])}
    </div>
    <div class="prompt-block">
      <h4>참고 이미지 규칙</h4>
      ${renderList([
        ...
        "참고 이미지의 구도를 그대로 베끼기보다, 판매용 홍보물에 맞게 더 정돈된 장면으로 재구성한다.",
        "이미지 안에 글자를 넣어야 한다면 짧고 선명한 문구만 사용한다.",
      ])}
    </div>
    <div class="prompt-block">
      <h4>금지사항</h4>
      ${renderList([
        ...
        "브랜드명 글자 오류, 깨진 한글, 읽기 어려운 작은 문구 금지",
    ...
    ${renderList([
      ...
      "브랜드명은 상단 중앙에 작고 고급스럽게 배치하고, 제품에는 작은 화이트 레터링만 넣어줘.",
